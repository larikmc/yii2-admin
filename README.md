...
- security log
- очистка кеша
- системный bootstrap для `admin`, `adminPanel` и защитных инвариантов root-пользователя `ID=1`
...
1. очистить опубликованные ассеты Yii (`web/assets/*`)
2. очистить кеш приложения (`php yii cache/flush-all`, если команда доступна)
...
'components' => [
    ...
    'urlManager' => [
        ...
        'rules' => [
            ...
            'rbac/<controller:[\w-]+>/<action:[\w-]+>/<id:\d+>' => 'admin/rbac/<controller>/<action>',
            'rbac/<controller:[\w-]+>/<action:[\w-]+>/<name:[\w-]+>' => 'admin/rbac/<controller>/<action>',
        ],
    ],
    'errorHandler' => [
        // Обязательно: чтобы 403/404/500 внутри админки рендерились шаблоном расширения.
...
1. администратор открывает `/admin/rbac/invite`
...
- генерация invite-ссылки доступна пользователям с ролью `admin`
- invite одноразовый
- системные ограничения для `ID=1` относятся к защите root-администратора (роль `admin` не снимается), а не к генерации invite

Совместимость с моделью пользователя:

- если в проекте нет колонки `username`, задайте `usernameField = 'email'`
- invite-регистрация автоматически подстроится и не будет выполнять SQL по `username`
- если `username` есть как отдельное поле, форма invite продолжает работать в классическом режиме (логин + email + пароль)
...
- генерация invite-ссылок доступна пользователям с ролью `admin`
...
1. очистку `web/assets/*`
2. очистку кеша приложения
...
- `user.loginUrl = ['/login']`
- правило `'login' => 'admin/auth/auth/login'`
- whitelist маршрутов входа/капчи/reset/invite в `beforeRequest`-guard
